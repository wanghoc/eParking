...
export function IPCameraStream({
    ...
    switch (protocol) {
      ...
      case "RTSP":
        ...
        if (rtspUrl) {
          ...
          const rtspPath = `rtsp://${auth}${ipAddress}:${
            port || 554
          }/live/cam/realmonitor?channel=1&subtype=0`;
          url = `/api/cameras/${cameraId}/stream?rtsp=${encodeURIComponent(
            rtspPath
          )}`;
        }
        break;
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (result.success && result.plate_number) {
          console.log(
            `[IPCamera ${cameraId}] Detected plate:`,
            result.plate_number,
            `(${(result.confidence! * 100).toFixed(1)}%)`
          );
          console.log(
            `[IPCamera ${cameraId}] Database result:`,
            result.database
          );
  ...
  const startPlateDetection = () => {
    ...
    console.log(
      `[IPCamera ${cameraId}] Started realtime plate detection (1s interval)`
    );
  };
      ...
      <div className="relative w-full h-full bg-gray-900 flex items-center justify-center">
        <div className="text-center p-4">
          ...
          <p className="text-gray-400 text-xs mt-2">
            {protocol === "Yoosee" && deviceId && <>Device ID: {deviceId}</>}
            {ipAddress && (
              <>
                IP: {ipAddress}:{port}
              </>
