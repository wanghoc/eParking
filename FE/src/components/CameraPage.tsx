...
import { CameraStream } from "./WebcamStream";
...
export function CameraPage() {
  ...
  const [selectedCameraForFull, setSelectedCameraForFull] = useState<
    number | null
  >(null);
  ...
  const handleAddWebcam = async () => {
    ...
    try {
      const selectedLot = parkingLots.find(
        (l) => l.id === quickWebcam.parking_lot_id
      );

      // Generate unique camera name with timestamp if no name provided
      const timestamp = Date.now();
      const defaultName = `Webcam ${quickWebcam.type} - ${
        selectedLot?.name || "Camera"
      } - ${timestamp}`;
  ...
  const organizeCamerasByParkingLot = () => {
    ...
    const activeCameras = camerasToUse.filter(
      (cam) => cam.status === "Hoạt động"
    );
        ...
        <div className="relative z-10 flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-4 lg:space-y-0">
          <div>
            ...
            <p className="text-cyan-100 text-base lg:text-lg drop-shadow-md">
              Hệ thống nhận diện biển số - Đại học Đà Lạt
            </p>
          </div>
                        ...
                        {row.map((camera) => {
                          return (
                            <div
                              ...
                              onClick={() =>
                                setSelectedCameraForFull(camera.id)
                              }
                            >
                      ...
                      <tbody className="bg-white divide-y divide-gray-200">
                        {cameras.map((camera) => (
                          ...
                          >
                            ...
                            <td className="px-6 py-6 whitespace-nowrap">
                              <span
                                className={`inline-flex px-3 py-1 text-xs font-semibold rounded-full ${getConnectionColor(
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setEditingCamera({ ...editingCamera, type: e.target.value })
                  }
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500"
                >
            ...
            <div className="p-6 space-y-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setQuickWebcam({ ...quickWebcam, name: e.target.value })
                  }
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button
                    onClick={() =>
                      setQuickWebcam({ ...quickWebcam, type: "Vào" })
                    }
                  ...
                  <button
                    onClick={() =>
                      setQuickWebcam({ ...quickWebcam, type: "Ra" })
                    }
                    className={`px-4 py-3 rounded-lg border-2 font-medium transition-all ${
                      quickWebcam.type === "Ra"
