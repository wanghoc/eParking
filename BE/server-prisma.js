...
async function getFeePerTurn() {
  try {
    ...
    return feeSettings
      ? parseFloat(feeSettings.setting_value)
      : DEFAULT_FEE_PER_TURN;
  } catch (error) {
...
app.post("/api/register", async (req, res) => {
  ...
  try {
    ...
    if (existingUser) {
      return res
        .status(409)
        .json({ message: "Email or MSSV already registered" });
    }
      ...
      if (session.exit_time) {
        if (session.payment_status === "Da_thanh_toan") {
          logs.push({
            ...
            message: `Đã thanh toán ${Number(session.fee).toLocaleString(
              "vi-VN"
            )}₫ cho xe ${session.vehicle.license_plate}`,
...
app.post("/api/vehicles", async (req, res) => {
  ...
  if (!user_id || !license_plate) {
    return res
      .status(400)
      .json({ message: "Vui lòng điền thông tin người dùng và biển số xe" });
  }
  ...
  try {
    ...
    res
      .status(201)
      .json({ message: "Vehicle registered", vehicleId: vehicle.id });
  } catch (err) {
    console.error(err);
...
app.delete("/api/vehicles/:vehicleId", async (req, res) => {
  ...
  try {
    ...
    if (openSession) {
      return res
        .status(400)
        .json({ message: "Cannot delete vehicle with open parking session" });
    }
...
app.post("/api/wallet/topup", async (req, res) => {
  ...
  if (!user_id || !amount) {
    return res
      .status(400)
      .json({ message: "Vui lòng điền thông tin người dùng và số tiền" });
  }
...
app.post("/api/cameras", async (req, res) => {
  ...
  if (!name || !type) {
    return res
      .status(400)
      .json({ message: "Vui lòng điền tên và loại camera" });
  }
...
app.post("/api/cameras/test-connection", async (req, res) => {
  const { ip_address, port, username, password, protocol, rtsp_url, http_url } =
    req.body;
  ...
  try {
    ...
    if (protocol === "RTSP" && !rtsp_url && !ip_address) {
      return res.status(400).json({
        message: "RTSP URL or IP address is required for RTSP protocol",
      });
    }

    if (protocol === "HTTP" && !http_url && !ip_address) {
      return res.status(400).json({
        message: "HTTP URL or IP address is required for HTTP protocol",
      });
    }
...
app.get("/api/cameras/:id/stream", async (req, res) => {
  ...
  try {
    ...
    switch (streamProtocol) {
      case "Yoosee":
        // Yoosee snapshot URL
        streamUrl = `http://${camera.ip_address}:${
          camera.port || 8000
        }/snapshot.jpg`;
        break;
      ...
      case "HTTP":
        ...
        streamUrl =
          camera.http_url ||
          `http://${camera.ip_address}:${camera.port || 80}/videostream.cgi`;
        break;
      ...
      case "RTSP":
        ...
        const rtspUrl =
          camera.rtsp_url ||
          `rtsp://${camera.ip_address}:${camera.port || 554}/live/ch00_0`;
    ...
    try {
      ...
      if (axiosResponse.status === 200) {
        ...
        res.setHeader(
          "Content-Type",
          axiosResponse.headers["content-type"] || "image/jpeg"
        );
...
app.get("/api/dashboard/stats", async (req, res) => {
  ...
  try {
    const [vehiclesCount, currentParking, monthlyParking, wallet] =
      await Promise.all([
...
app.get("/api/admin/vehicles", async (_req, res) => {
  try {
    ...
    const formatted = vehicles.map((v) => ({
      ...
      status:
        v.parking_sessions && v.parking_sessions.length > 0
          ? "Đang gửi"
          : "Không gửi",
    }));
...
app.post("/api/parking-lots", async (req, res) => {
  ...
  if (!name || !capacity) {
    return res
      .status(400)
      .json({ message: "Tên bãi xe và sức chứa là bắt buộc" });
  }
...
app.get("/api/admin/parking-sessions/active", async (_req, res) => {
  try {
    ...
    const formatted = sessions.map((s) => ({
      ...
      balance: s.vehicle.user?.wallet
        ? Number(s.vehicle.user.wallet.balance)
        : 0,
    }));
...
app.post(
  "/api/admin/parking-sessions/:sessionId/confirm-cash",
  async (req, res) => {
...
app.listen(PORT, () => {
  console.log(
    `eParking backend (Prisma) listening on http://localhost:${PORT}`
  );
});
